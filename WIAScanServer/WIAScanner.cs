...
namespace WIAScanServer
{
            ...
            foreach (Property prop in scan.Properties)
            {
                switch (prop.PropertyID)
                {
                    ...
                    case 6151: //horizontal exent 
                        SetProperty(prop, (int)(8.27 * settings.dpi));
                        break;
                    case 6152: //vertical extent 
                        SetProperty(prop, (int)(11.69 * settings.dpi));
                        break;
                }
            }
        }
    ...
    public class Scan
    {
        public static List<WIAScanServer.Scanner.Page> StartScan(WIAScanServer.Scanner.Options opts)
        {
            ...
            settings.color = opts.pixelType;
            settings.dpi = opts.dpi;
            settings.adf = opts.feeder;
            settings.tryFlatbed = true;

            try
            {
                
                List<Image> list = WIAScanner.Scan(opts.source, settings);
                List<WIAScanServer.Scanner.Page> pages = new List<WIAScanServer.Scanner.Page>();
                foreach (Image img in list)
                {
                    ...
                    WIAScanServer.Scanner.Page page = new WIAScanServer.Scanner.Page();
                    page.data = GetDataURL(stream);
                    page.size = stream.Length;
                    page.id = pages.Count + 1;
                    pages.Add(page);
                    int index = 1;
                    img.Save("D:\\scan\\test-" + index++ + ".jpg", System.Drawing.Imaging.ImageFormat.Jpeg);
                    img.Dispose();
                }
        ...
        public static string GetDataURL(MemoryStream stream)
        {
            //return "<img src=\"data:image/jpeg;base64," + Convert.ToBase64String(stream.ToArray()) + "\" />";
        ...
        private static int SaveImages(List<Image> list)
        {
            ...
            String filename = path + "\\" + now.ToString("yy_MM_dd-H_mm_ss");
            ...
            if (list.Count > 0)
                Logger.Log("Images saved in " + path);
